...
int main() {
  ...
  while (!quit) { // Main while
    cout << "Type a valid command(ADD, MADD, REMOVE, DISPLAY, QUIT)" << endl;   // Prompt user for input
    ...
    if (strcmp(input, "REMOVE") == 0) {
      ...
      cin >> goal;
      cin.clear();
      if (!search(root, goal, goaln)) { // Check if a node with that value
...
void remove(node * &root, node * &goaln) {
  ...
  if (goaln == root) { // If there is nothing but the root remove the root
	...
	temp = temp->left;
      ...
      if(temp->right != NULL && temp != goaln->right){
	...
	temp->parent->left = NULL;
      ...
      if(temp == goaln->right && temp->right != NULL){
	goaln->right = temp->right;
	temp->right->parent = goaln;
      }
      delete temp;
      return;
    }
